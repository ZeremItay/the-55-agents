...
התיקייה מכילה קובץ `.md` לכל אחד מהסוכנים בצוות ([[yael]], [[yuval]], [[chen]]), עם YAML frontmatter שמגדיר את התפקיד, תיאור (`description`), הכלים המותרים (`tools`), והמודל. **מצב נוכחי (2026-05-13):** `yael.md` נכתב ופועל; `yuval.md` ו-`chen.md` עדיין לא נוצרו (עד שיתבקש). **משויך:** [[reuven]] אחראי על האדריכלות; כל סוכן יקבל את הקובץ שלו.

## Open Questions

- איזה tools תקבל [[chen]] כשתיווצר? (WebSearch + WebFetch כברירת מחדל, MCP חיפוש לפי הצורך?)
- איזה tools יקבל [[yuval]] כשייווצר? (Bash ל-Gemini CLI לייצור תמונות? Read/Write בלבד?)
